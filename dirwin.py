...
def parse_winfo(linfo):
    ...
    for i in linfo:
        ...
        if line.startswith("Windowid"):
            ...
            elif el.startswith("map state"):
                visibility = (el[11:].split())
                dinfo.update(visibility=visibility[0])
            else:
                el = nline
...
def get_desktop_info(xprop_output):
    x = xprop_output.find("-geometry")
    if x != -1:
        xo = xprop_output[x + 10:]
        xo = xo.replace("-", "+")  # ignore +0+0 (TODO)
        xx = xo.find("+")
        if xx != -1:
            ds = xo[:xx]
            ds = ds.split("x")
        ...
        for col in range(int(i["absoluteupperleftx"] / scale),
            ...
            for row in range(int(i["absoluteupperlefty"] / scale),
                ...
                try:
                    ...
                    print("out of range (%d,%d), clamping to (%d,%d)" %
                          (col - 1, row - 1, cc, rc))
...
# for desktop dimensions, switched to xwinfo since it seems to work better with some window managers

xcmd = ["xwininfo", "-root"]
xwinoutput = subprocess.check_output(xcmd).decode("utf-8")
desktop_geo = get_desktop_info(xwinoutput)


# for stack and other things
xcmd = ["xprop", "-root"]
...
for i in stack:
    ...
    if (winfo["title"] == "xfce4-panel"
            or winfo["visibility"] == "isunmapped"):
        continue
    infos.append(winfo)
    print(winfo)
#infos = infos[::-1]
